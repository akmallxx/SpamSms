...
class nyepam:
	...
	def mulai(self):
		try:
			...
			exit(f'{red}# {off}Dah selesai cuk, jangan lupa follow IG: {green}@akmall.kz')
...
print(f'{cyan}  ___ ___  _____\n',
	f'{cyan}|   Y   ||     | {white}Instagram: {green}@akmall.kz\n',
	f'{cyan}|   1   ||  {white}°{cyan}  | {white}author{yellow}: {green}Akmalz\n',
	f'{cyan} \_   _/{red}.{white}^-----^{red}---.---.-.--------.\n',
	f'{cyan}  |{white}:  {cyan}|{red}|   __|  _  |  _  |        |\n',
	f'{cyan}  |{white}::.{cyan}|{red}|--   |   __|___._|__|__|__|\n',
	f'{cyan}  `---´{red}`-----|__| {white}Spammer otp & call\n\n')
print(f'{purple}[+] {white}Example: {cyan}08xxx')
while True:
	try:
		a=input(f'{purple}-> {white}Nomornya:{yellow} ')
		asu=a[0:2]
		if a in(""," "):print(f'{red}[!] {white}Jangan Kosong Ajg')
		elif "08" not in asu:print(f'{red}[!] {white}Gunakan Nomer Dengan Awalan 08xxx')
		elif len(a)<=10:print(f'{red}[!] {white}Nomor harus lebih dari {red}10 {off}digit')
		elif len(a)>=15:print(f'{red}[!] {white}Nomornya kebanyakan cok, maksimal {red}14 {off}digit')
		else:
			b=a[1:12] 
			c="62"+b
			nyepam(b,a,c).mulai()
	...
	except (KeyboardInterrupt,EOFError):exit("{red}[!] {off}Exit")
